...
const sendSignUpOtp = asyncHandler(async (req, res) => {
    ...
    await OtpStore.create({
        ...
        otp,
        expiresAt,
        userData: { name, phone, dob, password }
    ...
    return res.status(200).json(
        new ApiResponse(200, "OTP send successfully", { email })
    )
})
...
const verifyOtp = asyncHandler(async (req, res) => {
    ...
    const { name, phone, dob, password } = otpData.userData;
    ...
    return res.status(201).json(
        new ApiResponse(
            201,
            "OTP verified, user registered & logged in successfully",
            {
                userId: user._id,
                user,
                accessToken,
...
const resendOtp = asyncHandler (async (req,res) => {
    ...
    console.log(email);
    let otpRecord = await OtpStore.findOne({email});

    if(!otpRecord) {
        throw new ApiError(400,"No OTP request for the user");
    }
...
export { sendSignUpOtp, verifyOtp, loginUser, logoutUser, refreshAccessToken ,resendOtp };